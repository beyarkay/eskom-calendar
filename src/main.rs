...
fn create_calendar(csv_path: String, mis: &ManuallyInputSchedule) {
    ...
    for event in events.into_iter() {
        ...
        for change in &mis.changes {
            if change.stage == event.stage {
                if change.start < end_dt && change.finsh > start_dt {
                    let area_name = csv_path.replace("generated/", "").replace(".csv", "");
                    let description = format!(
                        "National Loadshedding from {} to {}\n\n{area_name} loadshedding from {} to {}\n\n---\nGenerated by Boyd Kane https://github.com/beyarkay/\n\nInformation scraped from {}\n\nCalendar compiled at {:?}",
                        change.start, change.finsh, start_dt, end_dt, change.source, chrono::offset::Local::now()
                    );
                    // eprintln!("national:   {}->{}", change.start, change.finsh);
                    // eprintln!("  area:     {}->{}", start_dt, end_dt);
                    let event_start = change.start.max(start_dt);
                    let event_finsh = change.finsh.min(end_dt);
                    // eprintln!(
                    //     "  result:   {}->{} ({})",
                    //     event_start, event_finsh, change.stage
                    // );
                    let evt = Event::new()
